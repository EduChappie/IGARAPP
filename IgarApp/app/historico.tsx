...
export default function HistoricoScreen() {
  ...
  const montarAcaoHistorico = (id: string, data: any) => ({
    id,
    titulo: data.titulo || "",
    cidade: data.cidade || "",
    estado: data.estado || "",
    dataEvento: normalizarData(data.dataEvento || data.data),
    horaInicio: data.horaInicio || "",
    horaFim: data.horaFim || "",
    voluntariosNecessarios: data.voluntariosNecessarios || 0,
    voluntariosInscritos: data.voluntariosInscritos || 0,
    ongId: data.ongId || data.organizadorId || "",
    status: data.status || "finalizada",
  });

  const fetchHistoricoVoluntario = async () => {
    if (!user?.uid) return [];
    ...
    for (const participacaoDoc of participacoesSnapshot.docs) {
      ...
      const acaoData = historicoDoc.data();
      ...
      historicoData.push({
        participacao: {
          ...
          dataInscricao:
            participacaoData.dataInscricao?.toDate?.() || new Date(),
          status: participacaoData.status || "participando",
        },
        acao: montarAcaoHistorico(historicoDoc.id, acaoData),
      });
    }
  ...
  const fetchHistoricoOng = async () => {
    if (!user?.uid) return [];

    const historicoRef = collection(firestore, "historico");
    ...
    historicoSnapshot.forEach((historicoDoc) => {
      const acaoData = historicoDoc.data();
      const ongId = acaoData.ongId || acaoData.organizadorId || "";

      if (ongId !== user.uid) return;
  ...
  const fetchHistorico = async () => {
    ...
    try {
      ...
      const historicoData =
        user.tipo === "ong"
          ? await fetchHistoricoOng()
          : await fetchHistoricoVoluntario();

      historicoData.sort(
        (a, b) => b.acao.dataEvento.getTime() - a.acao.dataEvento.getTime()
      );
  ...
  const handleNavigateToEvent = (acaoId: string) => {
    router.push(`/historico-detalhes?id=${acaoId}`);
  };
      ...
      >
        <View style={localStyles.cardHeader}>
          <View style={localStyles.tagContainer}>
            <Text style={localStyles.tagText}>
              {item.acao.cidade}, {item.acao.estado}
            </Text>
          </View>

          <View style={[localStyles.statusBadge, localStyles.statusConfirmed]}>
            <Text style={localStyles.statusText}>Finalizada</Text>
          </View>
        </View>
        ...
        <View style={localStyles.volunteersInfo}>
          ...
          <Text style={localStyles.volunteersText}>
            {item.acao.voluntariosInscritos} de{" "}
            {item.acao.voluntariosNecessarios} voluntários
          </Text>
        </View>
    ...
    return (
      <View style={localStyles.emptyContainer}>
        ...
        <Text style={localStyles.emptyTitle}>
          Nenhuma ação finalizada encontrada
        </Text>
        <Text style={localStyles.emptyText}>
          O histórico só mostra ações depois que a ONG finaliza o evento.
        </Text>
      ...
      <View style={styles.mainContainer}>
        ...
        <View style={extra.topContentContainer}>
          ...
          <Text style={extra.subtitleDescriptionText}>
            Veja as ações finalizadas{"\n"}que você participou ou organizou.
          </Text>
        </View>
        ...
        <View style={localStyles.contentContainer}>
          {loading ? (
            ...
            <FlatList
              ...
              keyExtractor={(item) =>
                item.participacao?.id || item.acao.id
              }
