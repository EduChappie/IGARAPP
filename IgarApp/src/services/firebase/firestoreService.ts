...
export const acaoService = {
  ...
  async getHistoricoById(acaoId: string): Promise<Acao | null> {
    try {
      ...
      if (!historicoDoc.exists()) return null;
  ...
  async getAcoesByIds(acaoIds: string[]): Promise<Acao[]> {
    try {
      ...
      const promises = acaoIds.slice(0, 10).map(async (id) => {
        const acaoSnap = await getDoc(doc(firestore, "acoes", id));
        if (acaoSnap.exists()) return montarAcao(acaoSnap.id, acaoSnap.data(), "ativa");

        const historicoSnap = await getDoc(doc(firestore, "historico", id));
        if (historicoSnap.exists()) return montarAcao(historicoSnap.id, historicoSnap.data(), "finalizada");
...
export const userService = {
  async salvarPerfil(
    ...
    try {
      ...
      if (ongSnap.exists()) {
        await updateDoc(ongRef, { ...campos, updatedAt: serverTimestamp() });
      ...
      if (userSnap.exists()) {
        await updateDoc(userRef, { ...campos, updatedAt: serverTimestamp() });
        console.log("Perfil user atualizado:", userId);
        return;
      }

      await setDoc(userRef, { ...campos, createdAt: serverTimestamp() });
...
export const getHistoricoUsuario = async (userId: string) => {
  try {
    ...
    return participacoes.map((participacao) => ({
      participacao,
      acao: acoesMap.get(participacao.acaoId) || null,
    }));
  } catch (error: any) {
